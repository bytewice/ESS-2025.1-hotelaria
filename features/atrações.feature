...
Scenario (Service): Avaliação válida de atração

Given o usuário de CPF "08622635480" 
And a atração "Spa" existe no sistema
When é enviada uma requisição POST para "/api/atracoes/:nome/avaliacoes" com os dados:
  | nome       | Ana               |
  | nota       | 5                 |
  | comentário | Muito Relaxante   |
  | data       | 05/07/2025        |
...
Scenario (Service): Falha ao enviar avaliação sem nota
...
Scenario (Service): Listar atrações disponíveis com sucesso
...
Scenario (Service): Visualizar detalhes da atração "Spa"
...
Scenario (Service): Listar avaliações da atração "Spa"
